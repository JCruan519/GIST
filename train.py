...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            loss = loss_fn(output, target) + loss_fn(gist, target)*0.5 + F.kl_div(F.softmax(gist/3.0, dim=-1).log(), F.softmax(output/3.0, dim=-1), reduction='batchmean')*0.75+F.kl_div(F.softmax(output/3.0, dim=-1).log(), F.softmax(gist/3.0, dim=-1), reduction='batchmean')*0.75
